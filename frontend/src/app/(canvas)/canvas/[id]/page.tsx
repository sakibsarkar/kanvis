...
import { updateCanvasImage } from "@/components/Canvas/canvasFunctions";
...
const ShapeEditor: React.FC = () => {
  ...
  useEffect(() => {
    if (initialRender.current) {
      ...
      return; // Exit early on the initial render
    }
      ...
      if (scale > 0 && scale < 110) {
        const projectPayload = {
          ...
          canvas: {
            width,
            height,
            bgColor: data.data.canvas.bgColor,
          },
          isExtended: true,
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await updateCanvasImage(id as string, token!); // Replace with your API call
      } catch (error) {
  ...
  return (
    <div className="w-full h-[calc(100%-70px)] relative flex items-start justify-start bg-slate-100 gap-[20px]">
      ...
      <div className="flex w-[calc(100%-450px)] h-full flex-col justify-start items-start gap-[10px] ">
        <div className="w-full h-[80px] bg-white flex items-center justify-between px-[30px] topBar">
          <TopBar />
        </div>
        <div
        ...
        >
          <div
            ...
            style={{
              ...
              backgroundColor: data?.data?.canvas.bgColor || "#ffffff",
